<template>
  ...
  <div :id="identification" style="width: 100%; height: 100%" class="panservice" :ref="identification">
    ...
    <div class="peizhi">
      <div class="p_left">
        <div class="p_l_M" v-if="stateM">
          ...
          <el-date-picker prefix-icon="1" v-model="timeStart" @change="timeStartFn" type="month"
            :picker-options="pickerOptionsMonth1" placeholder="选择日期" clear-icon="el-icon-error"> </el-date-picker>
      ...
      <div class="r_left">
        <img src="./img/导出标准大小.png" style="cursor: pointer" @click="exportFn" alt="" />
      </div>
    </div>
    <div class="tableBody">
      ...
      <el-table :data="tableData" stripe style="width: 100%" show-summary border :row-style="{ height: '40px' }"
        :summary-method="SumFn" :header-cell-style="{ padding: 0 + 'px' }"
        :cell-style="{ padding: 0 + 'px', color: '#000000' }" :header-row-style="{ height: '50px' }">
        <el-table-column prop="date" :label="stateR == 'month' ? '月份' : '年份'" width="254" align="center"
          :render-header="renderheader"> </el-table-column>
        <el-table-column prop="powerOutput" :label="stateR == 'month' ? '发电量|(万kWh)' : '发电量|(万kWh)'" sortable
          :formatter="totixdFn" align="center" :render-header="renderheader"> </el-table-column>
        <el-table-column prop="powerOngrid" :label="stateR == 'month' ? '上网电量|(万kWh)' : '上网电量|(万kWh)'" sortable
          :formatter="totixdFn" align="center" :render-header="renderheader">
        </el-table-column>
        <el-table-column prop="powerSelfuse" :label="stateR == 'month' ? '自发自用电量|(万kWh)' : '自发自用电量|(万kWh)'" sortable
          :formatter="totixdFn" align="center" :render-header="renderheader">
        </el-table-column>
        <el-table-column prop="feesSelfuse" :label="stateR == 'month' ? ' 自发自用电费|(万元)' : ' 自发自用电费|(万元)'" sortable
          :formatter="totixdFn" align="center" :render-header="renderheader">
        </el-table-column>
      </el-table>
    </div>
...
import { RadioButton, RadioGroup, Table, TableColumn, DatePicker } from "element-ui";
...
export default {
  ...
  data() {
    return {
      ...
      defaultValue: "",
      styleEle: null,
      iframeUrl: "/applicationview/content/view?appid=0d8c0264-2588-aad1-230c-392ab3b401f1&type=view&menuId=7f3afc2e-06b7-f7d4-ba3e-080d27a20ff4%233"
    };
  },
  watch: {
    tableData: {
  ...
  mounted() {
    //用于注册事件定义，不可删除
    let date1 = this.timeStart.getFullYear() + "-" + ((this.timeStart.getMonth() + 1) < 10 ? '0' + (this.timeStart.getMonth() + 1) : (this.timeStart.getMonth() + 1));
    let date2 = this.timeEnd.getFullYear() + "-" + ((this.timeEnd.getMonth() + 1) < 10 ? '0' + (this.timeEnd.getMonth() + 1) : (this.timeEnd.getMonth() + 1));
